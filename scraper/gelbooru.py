"""
gelbooru.py — Gelbooru metadata fetcher.

Gelbooru's "dapi" endpoint:
    https://gelbooru.com/index.php?page=dapi&s=post&q=index&json=1
    &tags=<tag>+sort:score:desc&limit=100&pid=<page>&api_key=...&user_id=...

Key facts (verified Jun 2026, see PLAN.md §1/§2):
  - JSON output now REQUIRES a free api_key + user_id (anonymous = HTTP 401).
    Register at https://gelbooru.com/index.php?page=account&s=options and set
    env vars  GELBOORU_API_KEY  /  GELBOORU_USER_ID.
  ...
  - Response JSON is a bare list of post objects (no envelope) when results exist,
    and `[]` (or a dict with "@attributes") when empty.
"""
...
class GelbooruFetcher(MetadataFetcher):
    def __init__(self):
        super().__init__(src=config.GELBOORU, min_score=config.THRESHOLDS.gelbooru_score)

    def build_url(self, tag: str, page: int) -> str:
        # Gelbooru wants tags space-separated but URL-encoded as '+'.
        ...
        # the URL (gelbooru would see duplicate keys and the request breaks).
        params: dict[str, Any] = {
            "json": "1",
            "tags": tags,
        ...
        # Auth is mandatory now; omit only if you want to see the 401.
    ...
    def parse_posts(self, payload: Any) -> list[dict]:
        # Gelbooru returns a list on success. An empty result can be [] or a
        ...
        posts: list[dict] = []
        for r in raw:
            # Gelbooru tags come as a single space-separated string.
            tags = r.get("tags", "")
